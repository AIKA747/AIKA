...
export async function postOrderAppPaymentStripeCreatePaymentIntent(
  ...
  return request<{
    code: number;
    msg: string;
    data: { 'customerId ': string; ephemeralKey: string; clientSecret: string; publishableKey: string };
...
export async function getOrderPublicPaymentStripeWebhook(body: string, options?: { [key: string]: any }) {
