...
export async function getContentAppStoryIdChapter(
  ...
  const { id: param0, ...queryParams } = params;
  return request<{ code: number; msg: string; data: API.ChapterPassDto }>(`/content/app/story/${param0}/chapter`, {
    method: 'GET',
    params: { ...queryParams },
    ...(options || {}),
  });
...
export async function getContentAppUserCollectStory(
  ...
  return request<{
    code: number;
    msg: string;
    data: { total: number; pageNum: number; pageSize: number; pages: number; list: API.StoryListDto[] };
...
export async function postContentAppUserCollectStory(
  ...
  return request<{ code: number; msg: string; data: { id: number } }>('/content/app/user-collect-story', {
    method: 'POST',
    headers: {
...
export async function deleteContentAppUserCollectStoryStoryId(
  ...
  return request<{ code: number; msg: string; data: string }>(`/content/app/user-collect-story/${param0}`, {
    method: 'DELETE',
    params: { ...queryParams },
    ...(options || {}),
  });
