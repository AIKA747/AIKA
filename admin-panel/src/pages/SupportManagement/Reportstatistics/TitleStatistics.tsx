import { getUserManageFeedbackTitleStatistics } from '@/services/api/yonghufankui';
...
export default ({ params = {} }: any) => {
  ...
  const { run } = useRequest(getUserManageFeedbackTitleStatistics, {
    manual: true,
    onSuccess(res) {
      if (res.code === 0) {
        setData(res.data);
  ...
  const options = useMemo(() => {
    if (data) {
      return {
        ...
        xAxis: {
          data: data?.map((ele) => ele.titleValue),
        },
        yAxis: {},
        series: [
          {
