...
export async function getOrderAppPaymentTotalAmount(options?: {
  [key: string]: any;
}) {
  return request<{ code: number; msg: string; data: number }>(
    '/order/app/payment/total-amount',
    {
      method: 'GET',
      ...(options || {}),
    },
...
export async function getOrderAppSubscriptionExpiredTime(options?: {
  [key: string]: any;
}) {
  return request<{ code: number; msg: string; data?: string }>(
    '/order/app/subscription/expired-time',
    {
      method: 'GET',
      ...(options || {}),
    },
...
/** 导出全部订单信息 生成一个 订单信息_{timestamp}.xslx  文件
导出字段信息：order no,  customer's name, email, phone number, item, amount, status, created time, payment method, payment amount, payment no., payment time; GET /order/manage/export */
export async function getOrderManageOpenApiExport(options?: {
  [key: string]: any;
}) {
  return request<Record<string, any>>('/order/manage/export', {
    method: 'GET',
    ...(options || {}),
  });
}

/** 订单详情 GET /order/manage/order/${param0} */
export async function getOrderManageOrderId(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getOrderManageOrderIdParams,
  options?: { [key: string]: any },
) {
...
/** 订单列表 GET /order/manage/orders */
export async function getOrderManageOrders(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getOrderManageOrdersParams,
  options?: { [key: string]: any },
) {
  return request<{
    code: number;
