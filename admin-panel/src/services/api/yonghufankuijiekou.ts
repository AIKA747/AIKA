...
/** 标记反馈回复已读 PUT /user/app/feedback/${param0}/read */
export async function putUserAppFeedbackIdRead(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.putUserAppFeedbackIdReadParams,
  options?: { [key: string]: any },
) {
  const { id: param0, ...queryParams } = params;
  return request<{ code: number; msg: string; data: string }>(
    `/user/app/feedback/${param0}/read`,
    {
      method: 'PUT',
      params: { ...queryParams },
      ...(options || {}),
